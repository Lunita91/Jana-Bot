...
await conn.sendButton2(m.chat, texto1, botname, res.videos[0].thumbnail, [
	...
	['Video 🎥', `${usedPrefix}mp4 ${text}`]
            ...
            if (command == "mp3") {
	 if (!text) return conn.reply(m.chat, `*🚩 Ingresa el titulo de un video o musica de YouTube.*`, m, )
...
if (size.split('MB')[0] >= limit) return conn.reply(m.chat,`El archivo pesa mas de ${limit} MB, se canceló la Descarga.`, m, fake,).then(_ => m.react('✖️'))
    ...
    if (command == "mp3doc") {
            if (!inputs) return conn.reply(m.chat, `*🚩 Ingresa el titulo de un video o musica de YouTube.*`,  m, m,)
    await m.react('🕓')
    let res = await yts(text)
    let vid = res.videos[0]
    let q = '128kbps'
...
thumbnail: await (await fetch(vid.thumbnail)).buffer()}}}, { quoted: m })
       await m.react('✅')
        ...
        await conn.reply(m.chat,`*☓ Ocurrió un error inesperado*`,  m, m,).then(_ => m.react('✖️'))
        console.error(error)
    }}}
